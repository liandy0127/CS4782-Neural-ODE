...
def build_spiral_dataset(
    *,
    ...
    # 1) Full theta‐grid and initial obs_t
    ...
    if subsample is not None and subsample < window:
        if clustered_sampling:
            ...
            # randomly pick exactly #subsample unique indices
...
def build_changing_freq_sine_dataset(
    *,
    ...
    # phi(t) = 2π [ f0*(t−t_start) + (1/2)(f1−f0)((t−t_start)² / delta t) ]
    rel = full_t - t_start
...
class ODEDynamics(nn.Module):
    """Latent ODE function  dx/dt = f_theta(z, t)."""
...
def plot_latent_trajectories(
    ...
    dim2: int = 1,
    n_trajs: int = 10,
    out_path: str = "latent_trajectories.png",
):
    """
    Plot the first two dims of each latent trajectory, inferred from
    your observed windows via the RNN encoder + latent ODE.
    """
    B, K, _ = data_obs.shape
    plt.figure(figsize=(6,6))

    encoder.eval()
    func.eval()

    t_tensor = torch.tensor(t_obs, dtype=torch.float32, device=device)  # (K,)

    with torch.no_grad():
        for i in range(min(n_trajs, B)):
            # 1) grab one observed window
            x_seq = torch.tensor(data_obs[i], dtype=torch.float32, device=device)  # (K,2)

            # 2) run RNN encoder backwards over the K steps:
            #    build a fresh h of shape (1, hidden)
            hidden_size = encoder.hid_size  # from your EncoderRNN
            h = torch.zeros((1, hidden_size), device=device)

            for ti in reversed(range(K)):
                x_t = x_seq[ti].unsqueeze(0)   # (1,2)
                out_t, h = encoder(x_t, h)     # out_t: (1, 2*z_dim)

            # split into μ and log var, then pick miu as deterministic z0
            mu_q, logvar_q = out_t.chunk(2, dim=1)  # each is (1, z_dim)
            z0 = mu_q.squeeze(0)                    # (z_dim,)

            # 3) integrate the latent ODE on the same t_obs grid
            z_traj = odeint(func, z0, t_tensor)     # (K, z_dim)
            z_np = z_traj.cpu().numpy()             # (K, z_dim)
    ...
    plt.xlabel(f"z[{dim1}]")
    plt.ylabel(f"z[{dim2}]")
    plt.title("Latent trajectories (first two dims)")
    plt.legend(loc="upper right", fontsize="small", ncol=2)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(out_path, dpi=300)
    plt.close()
    print(f"Saved latent trajectories to {out_path}")
...
def run_visualization(
    *,
    ...
    data_obs = obs_traj.detach().cpu().numpy()    # shape (B, K, 2)
    t_obs    = obs_time.detach().cpu().numpy()    # shape (K,)
    ...
    plot_latent_trajectories(
        ...
        dim1=0,
        dim2=1,
        n_trajs=min(30, data_obs.shape[0]),
        out_path=os.path.join(os.path.dirname(out_path), "latent_trajectories.png"),
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    p.add_argument("--visualize", type=eval, default=True,
    ...
    p.add_argument("--subsample", type=int, default=100,
                   help="optional subsample inside the window, should be < nsample")
    p.add_argument(
        "--dataset",
        type=str,
        choices=["spiral", "sinusoid"],
        default="spiral",
        help="which synthetic dataset to train on: 'spiral' or 'sinusoid'"
    )
    return p.parse_args()
